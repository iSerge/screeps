      ...
      room.find(FIND_STRUCTURES, {
        filter: (struct: Structure) => {
          if (
            ...
            utils.enqueueStructure(room, struct);
          }
          return false;
        }
      });
        ...
        if (700 < tower.store[RESOURCE_ENERGY]) {
          const closestDamagedStructure: Structure | null = tower.pos.findClosestByRange(FIND_STRUCTURES, {
            filter: (structure: Structure) => {
              return (
                (structure.structureType === STRUCTURE_RAMPART && structure.hits < Memory.maxRampartHits) ||
...
export const loop = () => {
  ...
  _.forOwn(Game.spawns, spawn => {
    ...
    if (spawn.spawning) {
      ...
      spawn.room.visual.text(`${Messages.CONSTRUCT_SYM} ${spawningCreep.memory.role}`, spawn.pos.x + 1, spawn.pos.y, {
        align: "left",
        opacity: 0.8
      });
    }
