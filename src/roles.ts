  ...
  public countCreeps() {
    _.forOwn(roles, (_role, roleName) => {
      _.forOwn(Memory.rooms, roomMem => {
        if (undefined === roomMem.creepCount) {
          roomMem.creepCount = {};
        }
  ...
  public spawn(room: Room, role?: CreepRole | BodySpec) {
    if (!role) {
      _.forOwn(roles, (r, name) => {
        if (name) {
          if (undefined === room.memory.creepCount) {
            room.memory.creepCount = {};
          }
          if (undefined === room.memory.creepCount[name]) {
            room.memory.creepCount[name] = 0;
          }
          if (room.memory.creepCount[name] < this.limit(name)) {
