...
export class Utils {
   ...
   * @return {number}
   */
  private static buildPriority(creep: Creep, site: ConstructionSite): number {
    let priority: number;
  ...
  public clearMemory() {
    ...
    _.forOwn(Memory.rooms, (room, name) => {
      if (name && !Object.prototype.hasOwnProperty.call(Game.rooms, name)) {
        console.log(`utils.clearMemory cleaning room ${name} memory`);
  ...
  public findConstructionSite(creep: Creep): ConstructionSite {
    return _.sortBy(Game.constructionSites, [
  ...
  public getEnergyStorageTarget(creep: Creep) {
    let target = this.getObjectById(creep.memory.energyTarget);
  ...
  public shiftStructure(creep: Creep, own: boolean): Structure | null {
    ...
    return this.getObjectById(needsRepair);
  }
  ...
  public updateInfrastructure() {
    if (undefined === Memory.rooms) {
      Memory.rooms = {};
      console.log("updateInfrastructure setup roooms memory segment");
    }
    _.forOwn(Game.rooms, room => {
      if (_.isUndefined(Memory.rooms[room.name])) {
        console.log(`updateInfrastructure setup rooom ${room.name} memory stucture`);
    ...
    if (_.isUndefined(Memory.maxWallHits)) {
      Memory.maxWallHits = 10000;
    }
    if (_.isUndefined(Memory.maxRampartHits)) {
      Memory.maxRampartHits = 3000;
    }
  }
